...
def processImageCrop(im_info, transformer, flow): #called by setup
  im_path = im_info[0]
  im_crop = im_info[1] 
...
class videoRead(caffe.Layer): # start of caffe layer definition
  ...
  def setup(self, bottom, top):
    ...
    #set image_mean. Makes sure to change me when using a different dataset
    if self.flow:
      image_mean = [128, 128, 128]
      self.transformer.set_is_flow('data_in', True)
    else:
      image_mean = [103.939, 116.779, 128.68] #remember to change me for different dataset 
...
class videoReadTrain_RGB(videoRead):
  ...
  def initialize(self):
    self.train_or_test = 'train'
    self.flow = False
    self.buffer_size = train_buffer  # 24 #num videos processed per batch  
    self.frames = train_frames  #16   #length of processed clip 
    self.N = self.buffer_size*self.frames #384
    self.idx = 0
    self.channels = 3
    self.height = 227
    self.width = 227
